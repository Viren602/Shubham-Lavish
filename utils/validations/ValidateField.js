...
export const isValidateSortCode = (value) => {
...
export const isValidateExactDigit = (value) => {
...
export const niNumberValidatieon = (value) => {
  const pattern = /^[A-Za-z]{2}\d{6}[A-Za-z]$/;
  if (!pattern.test(value)) {
    return false;
  } else {
    return true;
...
export const isValidateForLessOrEq = (value, compareValue) => {
  if (Number(value) <= Number(compareValue) && Number(value) > 0) {
    return true;
  } else {
    return false;
  }
